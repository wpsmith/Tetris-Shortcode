...
((function ( $ ) {
  ...
  $.widget('aerolab.blockrain', {
    ...
    status: function() {
      if (this._board.paused) { return 'paused'; }
      if (this._board.gameover) { return 'gameover'; }
      if (this._board.started) { return 'started'; }
      return 'nogame';
    },

    pause: function() {
      if (this._board.paused) { this._board.paused = false; }
      else { this._board.paused = true };
    },
    ...
    preview: function(nextShape) {
      console.log('change preview to show: ');
      console.log(nextShape);
      //this._$previewShape.ht
      nextShape.preview();
    },
     ...
     */
    _drawBlock: function(x, y, blockType, falling, ctx) {
      ...
      if( color instanceof Image ) {
        ...
        ctx.drawImage(color, 0, 0, color.width, color.height, x, y, this._block_size, this._block_size);

      }
      else if( typeof color === 'string' )
      {
        ctx.fillStyle = color;
        ctx.fillRect(x, y, this._block_size, this._block_size);
        ...
        if( typeof this._theme.innerShadow === 'string' ) {
          ...
          // Draw the borders
          ctx.strokeRect(x+1, y+1, this._block_size-2, this._block_size-2);
        }
        ...
        if( typeof this._theme.stroke === 'string' ) {
          ...
          // Draw the borders
          ctx.strokeRect(x, y, this._block_size, this._block_size);
        }
        if( typeof this._theme.innerStroke === 'string' ) {
          // Draw the inner dashes
          ctx.fillStyle = this._theme.innerStroke;
          ctx.fillRect(x+borderDistance, y+borderDistance, this._block_size-borderDistance*2, borderWidth);
          // The rects shouldn't overlap, to prevent issues with transparency
          ctx.fillRect(x+borderDistance, y+borderDistance+borderWidth, borderWidth, this._block_size-borderDistance*2-borderWidth);
        }
        if( typeof this._theme.innerSquare === 'string' ) {
          // Draw the inner square
          ctx.fillStyle = this._theme.innerSquare;
          ctx.globalAlpha = 0.2;
          ctx.fillRect(x+squareDistance, y+squareDistance, this._block_size-squareDistance*2, this._block_size-squareDistance*2);
        }
      }
    ...
    _drawPreviewBlock: function(x, y, blockType) {
      this._drawBlock(x, y, blockType, false, this._previewCtx);
    },
    ...
    _SetupShapeFactory: function(){
      ...
      function Shape(game, orientations, symmetrical, blockType) {
        ...
        $.extend(this, {
          ...
          preview: function() { console.log('do preview');
            var blocks = this.getBlocks(),
                i = 0;
            game._drawBackground(game._previewCtx);
            for (; i<this.blocksLen; i += 2) {
              game._drawPreviewBlock(this.x + blocks[i], this.y + blocks[i+1], this.blockType);
            }
          },
    ...
    _SetupBoard: function() {
      ...
      this._board = {
        ...
        // @todo Show Preview Shape (this.next)
        setNextShape: function() {
          this.next = this.nextShape();
          console.log('!game.options.noPreview: '+!game.options.noPreview);
    ...
    _createUI: function() {
      ...
      if (!game.options.noPreview) {
        ...
        game._$preview = $(
            ...
            '<div class="blockrain-preview-msg">' + this.options.showPreviewText + '</div>' +
        ...
        // Preview Canvas
        game._$previewCanvas = $('<canvas style="display:block; width:100%; height:100%; padding:0; margin:0; border:none;" />');
    ...
    _setupControls: function(enable) {
      ...
      function isStopKey(evt) {
        var cfg = {
          stopKeys: game.options.stopKeys || {37:1, 38:1, 39:1, 40:1, 27:1}
        };
